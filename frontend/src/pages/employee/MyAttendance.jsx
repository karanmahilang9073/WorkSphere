...
function MyAttendance() {
    ...
    const presentDays = attendance.filter(a => ['present', 'half-day'].includes(a.status?.toLowerCase())).length
                            ...
                            <table className="w-full text-left text-xs border-collapse">
                                ...
                                <tbody className="divide-y divide-slate-100 font-medium">
                                    {paginatedRecords.map((a) => {
                                        ...
                                        const isHalfDay = a.status?.toLowerCase() === 'half-day'
                                        const isIncomplete = a.status?.toLowerCase() === 'incomplete'
                                                        ...
                                                        <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200/80 flex flex-col items-center justify-center shrink-0">
                                                         ...
                                                         <div>
                                                             <div className="font-bold text-slate-900 text-xs">
                                                                 {formatDayName(a.date)}, {formatDateMonth(a.date)} {formatDateDay(a.date)}, {formatDateYear(a.date)}
                                                             </div>
                                                             <div className="text-[10px] text-slate-400 font-normal">
                                                                 {a.shiftType ? `${a.shiftType} Shift` : 'General Shift'} • {a.shiftStart || '10:00'} - {a.shiftEnd || '19:00'}
                                                             </div>
                                                         </div>
                                                     </div>
                                                 </td>
                                                 ...
                                                 <td className="py-3 px-4">
                                                     {a.overtimeHours > 0 ? (
                                                         <span className="inline-flex items-center gap-1 text-amber-800 font-bold bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200 text-[11px]">
                                                             <Zap className="w-3 h-3 text-amber-500" /> +{Number(a.overtimeHours).toFixed(1)} hrs
                                                         </span>
                                                     ) : (
